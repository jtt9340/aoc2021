...
int main(int argc, char *argv[])
{
    ...
    else // no filename; assume some type of testing mode
    {
        ...
        for (std::vector<AocTest>::iterator test_iter = tests_to_run.begin(); test_iter != tests_to_run.end(); ++test_iter)
        {
            ...
            {
                test_summary << "++Day " << test.get_day() << " Part " << test.get_part() << "-" << test.get_filename()
                             << " passed with result " << result << std::endl;
            }
            else
            {
                test_summary << "--Day " << test.get_day() << " Part " << test.get_part() << "-" << test.get_filename()
                             << " FAILED expected=" << test.get_expected_result() << " actual=" << result << std::endl;
            }
        }
